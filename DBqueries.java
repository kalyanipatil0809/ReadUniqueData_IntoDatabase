...
public class DBqueries {
	...
	public void insertIntoStudentsData(HashSet<StudentModel> uniqueSet) {
		...
		try (Connection connection = DBConnection.connect()) {
			String query = "Insert into studentsData  values(s_id,?,?,?,?,?,?,?,?)"; //for text file
			...
			if (uniqueSet != null) {
				for (StudentModel studentModel : uniqueSet) {
					...
					pstmt.setString(8, studentModel.getMarksModel().getFileExtension());
	...
	public void insertIntoStudentDetails(HashSet<StudentModel> uniqueSet) {
		...
		try (Connection connection = DBConnection.connect()) {
			String query = "Insert into studentDetails  values(student_id,?,?,?,?,?,?,?,?)"; //for excel file
			PreparedStatement pstmt = connection.prepareStatement(query);
